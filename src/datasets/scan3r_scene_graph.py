...
class SceneGraphPairDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        self.split = split
        self.use_predicted = cfg.sg_encoder.use_predicted
        self.sgaliner_model_name = cfg.sg_encoder.model_name
        self.scan_type = cfg.sg_encoder.scan_type
        self.sg_encoder_modules = cfg.sg_encoder.model.modules
        ...
        self.mode = 'orig' if self.split == 'train' else cfg.sg_encoder.val.data_mode
        ...
        self.img_patch_feat_dim = self.cfg.sg_encoder.model.img_patch_feat_dim
    ...
    def load3DSceneGraphs(self):
        # load scene graph
        if self.split == 'train':
            self.pc_resolution = self.cfg.sg_encoder.train.pc_res
        else:
            self.pc_resolution = self.cfg.sg_encoder.val.pc_res
        self.scene_graphs = {}
        for scan_id in self.all_scans_split:
            # Centering
...
if __name__ == '__main__':
    ...
    # scan3r_ds = SceneGraphPairDataset(cfg, split='val')
    # batch_size = 16
