...
class SubGenScan3R(Dataset):
    ...
    def __init__(self, cfg, split='train'):
        ...
        self.save_frame_idxs_name = "frame_idxs.txt"
    ...
    def __getitem__(self, data):
        ...
        while frame_cnt < len(frame_idxs):
            ...
            # save frame indexes of current subscan
            subscan_frame_idxs.append(frame_idxs)
            ...
            if subscan_pts.shape[0] >= max_pts_subscan:
                ...
                # record frame indexs of current subscan
                subscan_frame_idxs_arr = np.array(subscan_frame_idxs).astype(int).reshape(-1)
                np.save(osp.join(subscan_out_dir, self.save_frame_idxs_name), subscan_frame_idxs_arr)
                #reset 
                subscan_frame_idxs = []
