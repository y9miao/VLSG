...
class RoomRetrivalScore(SingleTester):
    def __init__(self, cfg):
        ...
        dataset, data_loader = get_val_dataloader(cfg, Dataset = PatchObjectPairXTemporalDataSet)
        ...
        # out 3D feature
        # self.features_3D = {}
        # self.out_3D_feature_dir = osp.join(self.scans_files_dir, '3D_features', self.method_name)
        # common.ensure_dir(self.out_3D_feature_dir)
    ...
    def test_step(self, iteration, data_dict):
        ...
        with torch.no_grad():
            start_time = time.time()
            patch_features_2D = self.model.forward2DImage(data_dict['temporal'])
            self.img_encoding_time += time.time() - start_time
            self.total_img += data_dict['temporal']['batch_size']
        
        output_dict = self.model_forward(data_dict['temporal'])
        return output_dict
    
    def eval_step(self, iteration, data_dict, output_dict):
        
        # get results
        embs = torch_util.release_cuda_torch(output_dict) 
        data_dict = torch_util.release_cuda_torch(data_dict['temporal'])
        ...
        for batch_i in range(batch_size):
            ...
            # patch_obj_sim
            patch_features_pb_norm = F.normalize(embs['patch_features'][batch_i], dim=-1)
            obj_features_pb_norm = F.normalize(embs['obj_features'][batch_i], dim=-1)
            ...
            self.room_retrieval_time += time.time() - start_time
            
            ## get image patch result
    ...
    def after_test_epoch(self):
        ...
        # save process time
        process_time_file = osp.join(self.output_dir, 'process_time.txt')
        with open(process_time_file, 'w') as f:
            f.write('img_encoding_time: {:.3f}s, room_retrieval_time: {:.3f}s, total_img: {} \n'.format(
                self.img_encoding_time, self.room_retrieval_time, self.total_img))
